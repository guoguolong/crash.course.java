...
void createDG(MGraph *G) {
    // 输入图含有的顶点数和弧的个数
    printf("请输入顶点数量："); scanf("%d", &(G->vexnum));
    printf("请输入弧数量："); scanf("%d", &(G->arcnum));

    // 依次输入顶点本身的数据
    printf("依次输入 %d 个顶点本身的数据：\n", G->vexnum);
...
void buildGraph(MGraph *G) {
    // 选择图的类型
    printf("请输入要创建的图的类型(0-DG、1-DN、2-UDG、3-UDN)：");
    scanf("%d", &(G->kind));
...
int main() {
    MGraph graph; // 建立一个图的变量
    buildGraph(&graph); // 调用创建函数，传入地址参数
    display(graph); // 输出图的二阶矩阵
